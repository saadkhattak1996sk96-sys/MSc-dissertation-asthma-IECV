# =====================================================================
# fit_ge4_debray_all_folds.R
#
# GE4 (>=4 exacerbations) — TRUE DEBRAY IECV PIPELINE, v3 fold structure
#
# Companion to the GE2 Debray pipeline (7_1-7_6). Same method, applied
# to the rarer outcome: one shared set of predictor coefficients frozen
# across all folds, with only the intercept re-estimated fresh for each
# held-out fold. Predictor set differs from GE2 per Blakey's own
# published GE4 model, which does not include Eczema, GERD, or NSAID
# use (unlike GE2's formula).
#
...
#   3. Re-estimate a fresh intercept for the held-out fold via the
...
#
# Missing data (BMI, PEF, eosinophil, smoking) handled via multiple
# imputation (m = 10, single-core mice() — not futuremice(), which was
# found to silently produce non-diverse imputed copies on this cluster;
# see project history), performed separately within training and
# validation data of each fold to avoid information leakage.
#
# Method notes:
#   - C-statistic: median across the 10 imputed copies (Marshall 2009),
#     SE via 200-repetition bootstrap on resampled rows/outcome.
#   - Calibration slope: pooled across imputed copies via Rubin's rules
#     (Wood 2015).
#   - O:E ratio: under true Debray this is a mathematical identity
#     (~1.0) rather than an independent estimate, since the held-out
#     fold's intercept is re-estimated from its own observed outcome.
#     Retained as a confirmation recalibration behaved correctly, not
#     as an informative performance measure.
#
# Imputation diversity check: verifies genuine variation across all
# four imputed variables, compared pairwise across every copy (not a
# single variable/pair), before any model is fit.
#
# Output: one row per fold (fit statistics) plus each fold's frozen
# shared coefficients, saved to disk for the pooling, forest plot,
# meta-regression, and final-model scripts.
# =====================================================================
...
# ---------------------------------------------------------------------
# Column selection: outcome, fold label, and GE4 predictors per
# Blakey's published GE4 model (excludes Eczema/GERD/NSAIDs, unlike
# GE2's predictor set)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Imputation method specification: PMM for the three continuous
# predictors with genuine missingness, polytomous regression for
# smoking status. Categorical derivatives (bmi_cat, pef_cat, eos_cat)
# deliberately excluded here -- constructed after imputation from the
# completed continuous values, so a patient's category can never
# disagree with their own imputed continuous value.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# Derive categorical predictors from completed continuous values,
...
#   - Shared-only formula (no intercept): design matrix for the
#     held-out fold once coefficients are frozen
# ---------------------------------------------------------------------
...
results_dir <- "/users/hlskhatt/outputs/ge4_debray_v3_fold_results"
dir.create(results_dir, showWarnings = FALSE, recursive = TRUE)

full_results_ge4_debray <- data.frame()
...
for (fold in all_folds) {
  ...
  train_raw <- year1 %>% filter(iecv_fold_v3 != fold, !is.na(blakey_outcome_ge4)) %>% base_select()
  valid_raw <- year1 %>% filter(iecv_fold_v3 == fold, !is.na(blakey_outcome_ge4)) %>% base_select()
  train_raw$iecv_fold_v3 <- factor(train_raw$iecv_fold_v3)

  # --- Multiple imputation, training data (m = 10, single-core) ---
  imp_train <- mice(train_raw, m = 10, method = meth_spec(train_raw),
                     maxit = 5, seed = 123, printFlag = FALSE)

  cat("--- mice() logged events, TRAINING data, fold", fold, "---\n")
  print(imp_train$loggedEvents)

  # --- Verification: confirm imputed copies are genuinely diverse
  #     across all four imputed variables, all copies pairwise ---
  check_vars <- c("Eosinophil_clean", "BMI_clean", "ba_PF_Percent_Pred", "Smoking_clean")
  comp1 <- complete(imp_train, 1)
  total_diff <- 0
  for (v in check_vars) {
    for (k in 2:imp_train$m) {
      compk <- complete(imp_train, k)
  ...
  if (total_diff == 0) stop("Imputed copies are not sufficiently diverse in fold ", fold, " -- halting.")
  ...
  #     intercepts (they represent the training pile's baseline risk,
  #     not the held-out fold's) ---
  ...
  # --- Multiple imputation, validation (held-out) data, independent
  #     of the training imputation ---
  imp_valid <- mice(valid_raw, m = 10, method = meth_spec(valid_raw),
                     maxit = 5, seed = 123, printFlag = FALSE)

  cat("--- mice() logged events, VALIDATION data, fold", fold, "---\n")
  print(imp_valid$loggedEvents)
  ...
  for (i in 1:imp_valid$m) {
    ...
    # Row-count check: confirms model.matrix has not silently dropped
    # any patient (which would happen if an unexpected NA remained).
    if (nrow(mm_no_int) != nrow(valid_i)) {
      stop("Fold ", fold, ": model.matrix dropped rows -- expected ",
           nrow(valid_i), " got ", nrow(mm_no_int))
    }
  ...
  fold_result <- data.frame(
    ...
    n_events_valid = sum(actual_outcome, na.rm = TRUE),
  ...
  full_results_ge4_debray <- rbind(full_results_ge4_debray, fold_result)
  ...
  saveRDS(list(fold = fold, shared_coefs = shared_coefs,
          ...
          file.path(results_dir, paste0("ge4_debray_v3_", fold, "_result.rds")))

  saveRDS(full_results_ge4_debray, "/users/hlskhatt/outputs/ge4_debray_v3_all_folds_summary.rds")
  saveRDS(shared_coefs_by_fold, "/users/hlskhatt/outputs/ge4_debray_v3_shared_coefs_by_fold.rds")

  cat("COMPLETED FOLD:", fold, "\n")
}

cat("\n\n=== ALL 7 FOLDS COMPLETE ===\n")
print(full_results_ge4_debray, row.names = FALSE)
