# =====================================================================
# pool_ge4_debray_bayesian_primary.R
#
# GE4 True Debray — cross-fold Bayesian random-effects pooling.
# Primary pooling method per the pre-specified analysis plan: one
# justified, citable method per performance measure. Identical
# specification to the GE2 Debray pooling script.
#
...
#
# Input:  ge4_debray_v3_all_folds_summary.rds (7-fold results table)
# Output: ge4_debray_v3_pooled_bayesian_results.rds
# =====================================================================
...
# ---------------------------------------------------------------------
# Convergence check: printed explicitly rather than assumed. Target
# psrf (potential scale reduction factor) is 1.05; values notably
# above that indicate the chains have not converged and the pooled
# result should not be trusted without further investigation.
# ---------------------------------------------------------------------
cat("--- Convergence check (psrf), calibration slope model ---\n")
print(fit_slope$psrf)
...
cat("\n=== TRUE DEBRAY GE4 — POOLED RESULTS ===\n")
print(final_summary)

saveRDS(list(fit_cstat = fit_cstat, fit_slope = fit_slope, slope_summary = slope_summary,
             final_summary = final_summary),
        "/users/hlskhatt/outputs/ge4_debray_v3_pooled_bayesian_results.rds")
