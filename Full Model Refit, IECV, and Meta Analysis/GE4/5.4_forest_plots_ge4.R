...
pooled <- readRDS("/users/hlskhatt/outputs/ge4_v3_pooled_bayesian_results.rds")
...
# --- C-statistic: CIs built on logit scale, then back-transformed ---
...
p_cstat <- make_forest_plot(cstat_data, pooled$fit_cstat$est, pooled$fit_cstat$ci.lb, pooled$fit_cstat$ci.ub,
                             "GE4 Full Refit (v3 folds) — C-statistic by Fold (IECV)", "C-statistic")
ggsave(file.path(output_dir, "GE4_v3_forest_cstat.png"), p_cstat, width = 8, height = 5, dpi = 300)

# --- Calibration slope: untransformed, as per the pooling method used ---
...
p_slope <- make_forest_plot(slope_data, pooled$slope_summary["mu","Median"], pooled$slope_summary["mu","Lower95"], pooled$slope_summary["mu","Upper95"],
                             "GE4 Full Refit (v3 folds) — Calibration Slope by Fold (IECV)", "Calibration Slope")
p_slope <- p_slope + geom_vline(xintercept = 1, linetype = "dashed", color = "grey50")
ggsave(file.path(output_dir, "GE4_v3_forest_slope.png"), p_slope, width = 8, height = 5, dpi = 300)

# --- O:E ratio: CIs built on log scale, then back-transformed ---
oe_data <- data.frame(
  fold = fixed_results$fold,
  estimate = log(fixed_results$oe_mean),
  lower = log(fixed_results$oe_mean) - 1.96 * (fixed_results$oe_se / fixed_results$oe_mean),
  upper = log(fixed_results$oe_mean) + 1.96 * (fixed_results$oe_se / fixed_results$oe_mean)
)
oe_data$estimate <- exp(oe_data$estimate)
oe_data$lower <- exp(oe_data$lower)
oe_data$upper <- exp(oe_data$upper)

p_oe <- make_forest_plot(oe_data, exp(pooled$oe_summary["mu","Median"]), exp(pooled$oe_summary["mu","Lower95"]), exp(pooled$oe_summary["mu","Upper95"]),
                          "GE4 Full Refit (v3 folds) — O:E Ratio by Fold (IECV, exploratory)", "O:E Ratio")
p_oe <- p_oe + geom_vline(xintercept = 1, linetype = "dashed", color = "grey50")
ggsave(file.path(output_dir, "GE4_v3_forest_oe.png"), p_oe, width = 8, height = 5, dpi = 300)

cat("Saved: GE4_v3_forest_cstat.png, GE4_v3_forest_slope.png, GE4_v3_forest_oe.png\n")
