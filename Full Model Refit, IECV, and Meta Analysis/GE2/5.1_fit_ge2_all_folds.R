...
build_split_v2 <- function(year1, held_out_fold, outcome_col) {
  ...
  train_raw <- year1 %>% filter(iecv_fold_v2 != held_out_fold, !is.na(.data[[outcome_col]])) %>% base_select()
  valid_raw <- year1 %>% filter(iecv_fold_v2 == held_out_fold, !is.na(.data[[outcome_col]])) %>% base_select()
...
results_dir <- "/users/hlskhatt/outputs/ge2_fold_results"
if (!dir.exists(results_dir)) dir.create(results_dir, recursive = TRUE)

full_results_ge2 <- data.frame()
...
for (fold in all_folds) {
  ...
  full_results_ge2 <- rbind(full_results_ge2, fold_result)
