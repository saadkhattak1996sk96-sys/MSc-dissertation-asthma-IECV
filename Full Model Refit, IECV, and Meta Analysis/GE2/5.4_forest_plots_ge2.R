...
make_forest_plot <- function(data, pooled_est, pooled_lb, pooled_ub, title, xlab, log_scale = FALSE) {
  ...
  p <- ggplot(plot_data, aes(x = estimate, y = label, color = type)) +
  ...
  if (log_scale) p <- p + scale_x_continuous(trans = "log")
  p
}
...
p_cstat <- make_forest_plot(cstat_data,
                             pooled$fit_cstat$est, pooled$fit_cstat$ci.lb, pooled$fit_cstat$ci.ub,
...
p_slope <- make_forest_plot(slope_data,
                             pooled$slope_summary["mu","Median"], pooled$slope_summary["mu","Lower95"], pooled$slope_summary["mu","Upper95"],
...
pooled_oe_est <- exp(pooled$oe_summary["mu","Median"])
pooled_oe_lb <- exp(pooled$oe_summary["mu","Lower95"])
pooled_oe_ub <- exp(pooled$oe_summary["mu","Upper95"])
p_oe <- make_forest_plot(oe_data, pooled_oe_est, pooled_oe_lb, pooled_oe_ub,
